 ...
 *
 * This function itself is browser-safe — it only ever performs a fetch()
 * to the server route below. All Nodemailer/SMTP logic lives entirely
 * server-side in that route; nothing server-only is imported here.
 */
export async function notifyDocumentReady(kind: DocumentKind, internshipId: string, documentId: string) {
  try {
    const res = await fetch('/api/admin/documents/notify', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type: kind, internshipId, documentId }),
    });
    const data = await res.json();
    if (!res.ok) {
