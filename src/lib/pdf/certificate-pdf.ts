...
function drawMedalRibbon(doc: jsPDF, cx: number, topY: number, color: string) {
  const w = 15;
  const drop = 34;
  doc.setFillColor(color);
  doc.triangle(cx - 24, topY, cx - 24 + w, topY, cx - 24, topY + drop, 'F');
  doc.triangle(cx + 24 - w, topY, cx + 24, topY, cx + 24, topY + drop, 'F');
}
...
export async function generateCertificatePdf(data: CertificateData): Promise<Blob> {
  ...
  // Soft off-white card background with a faint warm tint — reads as
  // premium cardstock rather than plain printer paper.
  doc.setFillColor('#FFFEF9');
  doc.rect(0, 0, W, H, 'F');

  // Subtle watermark of the official StoreShift seal, centered behind
  // everything, for a premium printed feel. Uses a pre-baked, fully-opaque
  // low-opacity asset rather than GState opacity on the alpha PNG — jsPDF's
  // handling of the two combined can leave a faint ghost of the
  // transparent-background pixels visible as a bounding box.
  try {
    const wmDataUrl = await loadImageAsDataUrl('/brand/certificate-seal-watermark.png');
    const wmH = 300;
    const wmW = wmH * (1024 / 901);
    doc.addImage(wmDataUrl, 'PNG', W / 2 - wmW / 2, H / 2 - wmH / 2 - 6, wmW, wmH);
  } catch {
  ...
  // Official seal — the StoreShift laurel emblem — hung like a medal,
  // centered in the signature/QR band.
  const laurelCx = W / 2;
  const laurelCy = bandY + 58;
  const emblemH = 96;
  const emblemW = emblemH * (1024 / 901);
  drawMedalRibbon(doc, laurelCx, laurelCy + emblemH / 2 - 14, INK);
  try {
    const sealDataUrl = await loadImageAsDataUrl('/brand/certificate-seal.png');
    doc.addImage(sealDataUrl, 'PNG', laurelCx - emblemW / 2, laurelCy - emblemH / 2, emblemW, emblemH);
  } catch {
    // fall back to a plain gold ring if the seal artwork can't load
    doc.setDrawColor(GOLD);
    doc.setLineWidth(1.4);
    doc.circle(laurelCx, laurelCy, emblemH / 2 - 10, 'S');
  }
