...
export default async function AttendancePage() {
  ...
  // status='active' only, so a completed intern's attendance history
  // went permanently blank.
  const { data: internship } = await supabase.from('internships').select('id').eq('intern_id', profile!.id).order('start_date', { ascending: false }).limit(1).maybeSingle();
  const { data: records } = internship
    ...
    : { data: [] };
  const { data: pendingRequests } = internship
    ? await supabase.from('attendance_rectification_requests').select('date, status').eq('internship_id', internship.id).eq('status', 'pending')
    : { data: [] };
  const pendingDates = new Set((pendingRequests ?? []).map((r: any) => r.date));
              ...
              {(records ?? []).map((r: any) => (
                <tr key={r.id} className="border-b border-ink-50 last:border-0">
                  ...
                  <td className="px-5 py-3 text-ink-400">
                    {r.note ?? '—'}
                    {pendingDates.has(r.date) && <span className="ml-2 text-xs text-amber-500">· correction pending admin review</span>}
                  </td>
                </tr>
              ))}
