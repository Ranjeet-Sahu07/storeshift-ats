...
export default async function InternGithubPage() {
  ...
  // status='active' only, so a completed intern's repo/PR history went
  // permanently blank.
  const { data: internship } = await supabase.from('internships').select('*').eq('intern_id', profile!.id).order('start_date', { ascending: false }).limit(1).maybeSingle();
