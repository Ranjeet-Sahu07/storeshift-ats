...
export default function MessagesPage() {
  ...
  async function load() {
    ...
    // status='active' only, so a completed intern lost access to their
    // mentor chat history entirely.
    const { data: internship } = await supabase.from('internships').select('mentor_id').eq('intern_id', user.id).order('start_date', { ascending: false }).limit(1).maybeSingle();
  ...
  async function send() {
    ...
    if (!error && data) {
      ...
      await notifyUser(supabase, mentor.id, 'New message from your intern', text.slice(0, 140), '/admin');
    }
    setText('');
  }
