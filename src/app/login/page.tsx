...
function LoginForm() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    const destination =
    next ??
    (profile && staffRoles.includes(profile.role)
    ? '/admin'
    : '/dashboard');
  ...
  return (
    ...
    Sign in to StoreShift
    </h1>
    
    <p className="text-sm text-ink-100/60">
    Official credentials sent to your email
    </p>
    </div>
    
    <form
    onSubmit={handleLogin}
    className="glass !bg-white/95 rounded-2xl p-6 shadow-2xl"
    >
    ...
    placeholder="rahul.int23@storeshift.in"
    />
    </div>
