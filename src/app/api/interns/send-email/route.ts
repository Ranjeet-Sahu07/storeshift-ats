...
export async function POST(req: NextRequest) {
  ...
  const html = brandedEmailShell(
    `${eyebrow('Message from StoreShift')}${heading(`Hi ${recipient.full_name},`)}${renderMessageParagraphs(message)}<p style="color:#9AA5A4;font-size:12px;margin-top:20px;">— ${escapeHtml(actorProfile.full_name)}, StoreShift</p>`
  );
