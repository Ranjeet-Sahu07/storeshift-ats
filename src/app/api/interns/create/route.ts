...
import { brandedEmailShell, eyebrow, heading, emailButton, credentialTable, infoCard, footerNote } from '@/lib/email/templates';
...
export async function POST(req: NextRequest) {
  ...
  if (mentorId) {
    const { data: mentor } = await admin.from('profiles').select('full_name').eq('id', mentorId).maybeSingle();
  ...
  const internshipRows: [string, string][] = [
    ['Role', roleTitle],
    ['Department', department],
    ['Start Date', resolvedStart.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })],
    ['Duration', `${durationMonths ? Number(durationMonths) : resolvedDuration} months`],
  ];
  if (mentorName) internshipRows.push(['Mentor', mentorName]);
  const details = infoCard('Your Internship Details', internshipRows);

  const closing = footerNote(
    "Once you're in, check your Dashboard for onboarding tasks, your assigned mentor's contact, and learning resources to get started. Questions any time — just reply to this email."
  );

  const html = brandedEmailShell(
    `${eyebrow('Congratulations 🎉')}${heading(`Welcome to StoreShift, ${fullName}!`)}${bodyHtml}${details}${credentialsBlock}${closing}`
  );
