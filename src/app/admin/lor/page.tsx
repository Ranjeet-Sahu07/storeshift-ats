...
export default function LorPage() {
  ...
  async function generate(internship: any) {
    ...
    try {
      ...
      // add one" client-side.
      const { data: lorId, error: idError } = await supabase.rpc('next_lor_id');
      if (idError || !lorId) throw new Error(idError?.message ?? "Couldn't generate an LOR ID");
      ...
      if (lorRow) {
        await notifyDocumentReady('lor', internship.id, lorRow.id);
        await notifyUser(supabase, internship.intern_id, 'Your letter of recommendation is ready', `LOR ${lorId} has been issued.`, '/dashboard/documents');
      }
  ...
  async function regenerate(lor: any) {
    ...
    try {
      ...
      if (internship) await notifyUser(supabase, internship.intern_id, 'Your letter of recommendation was updated', `LOR ${lor.lor_id} was regenerated.`, '/dashboard/documents');
