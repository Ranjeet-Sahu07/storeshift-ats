...
export default function CertificatesPage() {
  ...
  async function generateFor(internshipIds: string[]) {
    ...
    for (const id of internshipIds) {
      ...
      // Atomic sequence via RPC (see migration 008) — not "count rows,
      // add one" client-side, which races under concurrent generation
      // and after any row is ever deleted.
      const { data: certificateId, error: idError } = await supabase.rpc('next_certificate_id');
      if (idError || !certificateId) {
        toast.error(`Couldn't generate an ID for ${internship.profiles?.full_name}: ${idError?.message ?? 'unknown error'}`);
        continue;
      }
      ...
      try {
        ...
        if (certRow) {
          await notifyDocumentReady('certificate', internship.id, certRow.id);
          await notifyUser(supabase, internship.intern_id, 'Your certificate is ready', `Certificate ${certificateId} has been issued.`, '/dashboard/documents');
        }
        succeeded++;
      } catch (err: any) {
  ...
  async function regenerate(cert: any) {
    ...
    try {
      ...
      const { data: internshipRow } = await supabase.from('internships').select('intern_id').eq('id', cert.internship_id).maybeSingle();
      if (internshipRow) await notifyUser(supabase, internshipRow.intern_id, 'Your certificate was updated', `Certificate ${cert.certificate_id} was regenerated.`, '/dashboard/documents');
