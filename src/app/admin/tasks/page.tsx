...
export default function TasksPage() {
  ...
  async function load() {
    ...
    const [internRes, taskRes] = await Promise.all([
      supabase.from('internships').select('id, intern_id, role_title, profiles!internships_intern_id_fkey(full_name)').eq('status', 'active'),
  ...
  async function createTask() {
    ...
    const internship = internships.find((i) => i.id === form.internshipId);
    if (internship?.intern_id) await notifyUser(supabase, internship.intern_id, 'New task assigned', form.title, '/dashboard/tasks');
