...
export default function OfferLettersPage() {
  ...
  async function generate(internship: any) {
    ...
    try {
      ...
      // add one" client-side.
      const { data: offerId, error: idError } = await supabase.rpc('next_offer_id');
      if (idError || !offerId) throw new Error(idError?.message ?? "Couldn't generate an offer ID");
      ...
      if (offerRow) {
        await notifyDocumentReady('offer_letter', internship.id, offerRow.id);
        await notifyUser(supabase, internship.intern_id, 'Your offer letter is ready', `Offer letter ${offerId} has been issued.`, '/dashboard/documents');
      }
  ...
  async function regenerate(offer: any) {
    ...
    try {
      ...
      await notifyUser(supabase, internship.intern_id, 'Your offer letter was updated', `Offer letter ${offer.offer_id} was regenerated.`, '/dashboard/documents');
