...
## 14. Changelog — round 10 (client/server architecture fix: Nodemailer bundling)

- **Fixed `Module not found: Can't resolve 'fs'` on `/admin/offer-letters`
  and `/admin/lor`.** Root cause: both pages are Client Components that
  imported `renderTemplate` from `@/lib/email/service.ts` — the same file
  that (transitively, via Nodemailer) requires Node's `fs` module. Any
  import from that file, even of an unrelated named export, pulls the
  entire module — and everything it imports — into the browser bundle.
  `fs` doesn't exist in a browser, so the build failed.
- **`src/lib/email/service.ts` is now explicitly server-only** —
  `import 'server-only'` at the top turns any future accidental client
  import of this file into a clear build-time error instead of the
  confusing "Can't resolve 'fs'" message.
- **`renderTemplate` extracted into `src/lib/email/template.ts`** — pure
  string interpolation, zero dependencies, safe to import from anywhere
  including Client Components. `service.ts` re-exports it for other
  server code's convenience, but the two offending pages now import it
  directly from `template.ts`.
- **Document notification route relocated to `src/app/api/admin/documents/notify`**
  (previously `/api/documents/notify`) and tightened: it now explicitly
  checks the caller's role is one of the staff roles (previously it only
  checked *a* profile existed, not that they were staff), and request
  body validation/error handling around it is more defensive. `lib/documents.ts`'s
  `notifyDocumentReady()` — the only thing Client Components call — was
  updated to point at the new path; its own contract didn't change.
- **Full dependency-graph audit performed** (not just the two reported
  pages) — searched for every reachable import of `email/service.ts`,
  `nodemailer`, `notifyDocumentReady`, and `sendEmail` across `src`.
  Confirmed: `lib/documents.ts` (the shared client helper both pages use)
  never imported the server-only chain to begin with — it only ever does
  `fetch()` to API routes — so no splitting was needed there. Also swept
  for other Node-only modules (`fs`, `path`, `crypto`, `stream`, `net`,
  `tls`, `child_process`) reachable from any Client Component: none found.
- **Fixed the `getSession()` insecurity warning** where it actually
  mattered: `getCurrentProfile()` (used by the admin/dashboard layouts to
  decide *what a person is authorized to see*) now uses `getUser()`,
  which re-validates against the Supabase Auth server instead of trusting
  the cookie as-is. Middleware intentionally keeps `getSession()` — it
  only checks "does a session exist, redirect to `/login` if not," never
  makes a role/access decision from it, and every table's RLS remains the
  real enforcement boundary regardless — so re-verifying on every single
  navigation bought latency, not security, in that one specific spot.
- Run `npm install` after unzipping (added the `server-only` package).

## 15. Changelog — round 11 (public submission bug, link management, richer emails)

- **Fixed "new row violates row-level security policy for table
  applications" for public (non-logged-in) applicants.** The form
  previously did a plain client-side select-then-insert-or-update
  directly against `applications`, which depends on the
  `applications_public_insert` RLS policy being exactly right for the
  anon role at submission time — fragile for the single most important
  public-facing write in the app, and apparently drifted on the live
  database. Replaced with a `SECURITY DEFINER` RPC function
  (`submit_application`, adapted from the one you'd already written and
  tested — same shape, added `revoke/grant execute` so only `anon` and
  `authenticated` can call it, and a check that the declaration was
  actually accepted). It runs with elevated privileges, so it no longer
  depends on the caller's RLS grants at all, and it also makes the
  "same email on this link = correction" logic atomic — one round trip
  instead of a separate check and a separate write that could race.
  **Run migration 007** — this is the fix.
- **Application Links: Delete, per-link stats, and homepage control added.**
  - **Delete** — a link with a confirm step; if applications already
    reference it (foreign key), you get a clear message to deactivate
    instead of a raw database error.
  - **Applications vs. Selected counts**, shown per link — previously
    only total applications were visible, not how many actually
    converted to hires.
  - **"Show on careers homepage" is now an explicit per-link toggle**
    (`show_on_homepage`, defaults to **off**). Previously every active,
    non-expired link automatically appeared in "Currently Hiring" —
    now nothing shows there unless an admin opts it in, so you can share
    a link privately (e.g. a specific candidate, a referral) without it
    being publicly listed. Toggle is editable inline from the links
    table, and set-able at creation.
- **Emails made more informative and professional**, per your request —
  two new building blocks in `lib/email/templates.ts`:
  - `infoCard()` — a shaded, titled reference block (visually distinct
    from `credentialTable`, which stays reserved for actual
    login/password values so a reader's eye treats the two differently).
  - `footerNote()` — a closing note under the main content.
  - The **welcome email** now includes a "Your Internship Details" card
    (role, department, start date, duration, and mentor name if one's
    assigned) plus a closing note pointing to the dashboard for
    onboarding — not just bare credentials.
  - The **document-ready email** now shows a "Document Details" card
    (document type, role, department, and the actual reference ID —
    e.g. `SS-OFR-2026-0005`) and an appropriate closing note explaining
    *why* offer letters/LORs route through the dashboard while
    certificates link directly.
- Run **migration 007** (`supabase/migrations/007_*.sql`) — it's the one
  that matters this round.

## 16. Changelog — round 12 (Continue button bug, loading states)

- **Fixed: Continue button stopped working when a link had per-link
  required fields set** (e.g. Date of Birth, Address, Portfolio marked
  mandatory). Root cause: the admin-required check used
  `setError()` — a **react-hook-form-managed** error — for fields the
  zod schema itself considers optional. Every subsequent
  `trigger()`/`handleSubmit()` call runs the resolver against the whole
  schema and rewrites the *entire* errors object from that result; since
  the schema says those fields are fine, the resolver's own validation
  pass could clobber the manually-set error inconsistently between
  renders — which fields showed an error and whether the button
  considered the step "valid" could fall out of sync with each other.
  Rewrote the check to use a plain, separate React state object
  (`adminErrors`) that never touches react-hook-form's own error
  tracking, and switched from `watch()` to `getValues()` for the
  point-in-time read on click (the method react-hook-form's own docs
  recommend for exactly this "read the value right now" case, as
  opposed to `watch()`, which is meant for render-time subscriptions).
  Verified this was the only place in the codebase using this pattern.
- **Also fixed a latent double-click race** on the same Continue button —
  clicking twice quickly while the async `trigger()` validation was still
  resolving could start a second validation pass before the first
  finished. Added a `validating` guard with a spinner, applied to both
  the desktop and mobile sticky Continue buttons.
- **Loading states added** for `/apply/[code]` and
  `/certificate/verify` (Next.js shows these automatically during
  server-side data fetching), matching the branded spinner already used
  for `/admin` and `/dashboard`.
- Migration 007 note: your modified version (with `create function`
  instead of `create or replace function`, requiring the explicit
  `drop function` first) is functionally equivalent to what's in this
  repo — no further action needed on that front.
