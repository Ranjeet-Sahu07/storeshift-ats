...
create table application_links (
  ...
  -- Whether this link appears in the "Currently Hiring" section of the
  -- public careers homepage — an explicit admin decision, not automatic
  -- just because the link is active.
...
-- ============================================================================
-- PUBLIC APPLICATION SUBMISSION (SECURITY DEFINER RPC)
--
-- The application form calls this instead of inserting into `applications`
-- directly. Running as SECURITY DEFINER means it doesn't depend on the
-- anon role's RLS grants at submission time, and it makes the "same
-- email on this link = correction, not a duplicate" logic atomic (one
-- round trip, no race between a separate check and a separate write).
-- ============================================================================
...
begin
  ...
  if v_existing_id is not null then
    ...
    set
      full_name = p_full_name, email = p_email, phone = p_phone, date_of_birth = p_date_of_birth,
      gender = p_gender, address = p_address, city = p_city, state = p_state, college = p_college,
      degree = p_degree, branch = p_branch, graduation_year = p_graduation_year, cgpa = p_cgpa,
      tenth_percentage = p_tenth_percentage, twelfth_percentage = p_twelfth_percentage,
      graduation_percentage = p_graduation_percentage, skills = p_skills, preferred_role = p_preferred_role,
      resume_url = p_resume_url, portfolio_url = p_portfolio_url, github_url = p_github_url,
      linkedin_url = p_linkedin_url, questionnaire = p_questionnaire,
      declaration_accepted = p_declaration_accepted, declaration_accepted_at = p_declaration_accepted_at,
      status = 'submitted'
  ...
  -- (set_application_id, defined earlier in this file) generates it.
